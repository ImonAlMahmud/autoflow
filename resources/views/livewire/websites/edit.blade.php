            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <label class="p-4 rounded-xl border-2 cursor-pointer transition-all flex items-start gap-3 {{ $source_type === 'local' ? 'border-indigo-600 bg-indigo-50/50' : 'border-[#EAECF0] hover:bg-[#F9FAFB]' }}">
                        <input type="radio" wire:model.live="source_type" value="local" {{ $source_type === 'local' ? 'checked' : '' }} class="mt-1 text-indigo-600 focus:ring-indigo-500">
                        <div>
                    ...
                    <label class="p-4 rounded-xl border-2 cursor-pointer transition-all flex items-start gap-3 {{ $source_type === 'git' ? 'border-indigo-600 bg-indigo-50/50' : 'border-[#EAECF0] hover:bg-[#F9FAFB]' }}">
                        <input type="radio" wire:model.live="source_type" value="git" {{ $source_type === 'git' ? 'checked' : '' }} class="mt-1 text-indigo-600 focus:ring-indigo-500">
                        <div>
        ...
        <div class="bg-white rounded-2xl border border-[#EAECF0] shadow-xs p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    >
                        <option value="automatic" {{ $approval_mode === 'automatic' ? 'selected' : '' }}>Automatic (Auto-update content when validation passes)</option>
                        <option value="manual" {{ $approval_mode === 'manual' ? 'selected' : '' }}>Manual Review (Require human approval before applying)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        >
                            <option value="minutes" {{ $interval_unit === 'minutes' ? 'selected' : '' }}>Minute(s)</option>
                            <option value="hours" {{ $interval_unit === 'hours' ? 'selected' : '' }}>Hour(s)</option>
                            <option value="days" {{ $interval_unit === 'days' ? 'selected' : '' }}>Day(s)</option>
                            <option value="months" {{ $interval_unit === 'months' ? 'selected' : '' }}>Month(s)</option>
                        </select>
                    </div>
                </div>
            </div>
